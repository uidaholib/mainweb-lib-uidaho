---
title: Government Documents
section: Find
permalink: /find/govdocs/
layout: page
search: true
description: "Government documents research guide for locating federal and state publications through library collections and catalogs."
keywords: government documents; public records; research guide
page_nav:
    parent: /find/
    children:
---

The University of Idaho Library is the Regional Depository for U.S. Government Documents for the state of Idaho. The collection consists of more than 675,000 paper documents, 800,000 microforms, 200,000 maps, 7,500 CD-ROMs, and more than 25,000 online publications. This comprehensive and unique collection offers free access to more than 1.7 million items to all users. Unconstrained access to federal documents helps to inform and empower students and residents of Idaho to become better citizens.

<a href="https://libguides.uidaho.edu/Documents" class="btn btn-lg btn-clearwater my-2">Gov Docs Research Guide</a>

### Federal Depository Library Program

The Federal Depository Library Program was initiated by Congress in 1813. Today the Government Printing Office oversees the program under the authority of Title 44 of the United States Code. The University of Idaho Library joined the program in 1907. In 1967, the U of I Library became a regional depository, meaning it acquires 100% of items issued by GPO through the FDLP Program. As a regional depository, the library also oversees eight other selective depositories in the state of Idaho.

### Gov Doc Policies

The document types listed below can only be used in the Library.

#### GENERAL CATEGORIES

<ul>
	<li>Documents shelved in reference</li>
	<li>Climate data</li>
	<li>Statistical data</li>
	<li>Documents published before 1919</li>
</ul>

#### SPECIFIC AGENCIES

<table class="table table-striped">
		<thead>
		<tr>
			<th scope="col"><u>Call No.</u></th>
			<th scope="col"><u>Agency</u></th>
		</tr>
		</thead>
		<tbody>
		<tr>
			<td >A 13.</td>
			<td >Forest Service</td>
		</tr>
		<tr>
			<td>C 3. and C 56.</td>
			<td>Census Bureau</td>
		</tr>
		<tr>
			<td>I 19.</td>
			<td>U.S. Geological Survey</td>
		</tr>
		<tr>
			<td>I 29.</td>
			<td>National Park Service</td>
		</tr>
		<tr>
			<td>I 49.</td>
			<td>Fish and Wildlife Service</td>
		</tr>
		<tr>
			<td>L 2.</td>
			<td>Bureau of Labor Statistics</td>
		</tr>
		<tr>
			<td>SI 2.</td>
			<td>Bureau of American Ethnology</td>
		</tr>
		<tr>
			<td>S 1.1</td>
			<td>Department of State, Foreign Relations of the U.S.</td>
		</tr>
	</tbody>
</table>

#### LEGAL AND CONGRESSIONAL PUBLICATIONS

<table class="table table-striped">
	<tbody>
		<tr>
			<td>Y 1.1/2</td>
			<td>Serial Sets</td>
		</tr>
		<tr>
			<td >X 1.1 and X 1.1/A</td>
			<td >Congressional Records</td>
		</tr>
		<tr>
			<td>AE 2.110</td>
			<td>Slip Laws</td>
		</tr>
		<tr>
			<td>AE 2.111</td>
			<td>U.S. Statutes at Large</td>
		</tr>
		<tr>
			<td>AE 2.106</td>
			<td>Federal Registers</td>
		</tr>
		<tr>
			<td>AE 2.106/3</td>
			<td>Code of Federal Regulations</td>
		</tr>
	</tbody>
</table>

Exceptions to circulation policies may be made by government documents staff. Such exceptions may take 24 hours to grant depending on personnel availability. Please ask about exceptions at the Reference and Information Desk.
